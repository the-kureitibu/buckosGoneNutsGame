...
func handle_projectile_attack(delta):
	...
	if attack_state == EnemyStateManager.EnemyStates.CAN_ATTACK and !is_attacking:
		is_attacking = true
		var direction = (nav_target.global_position - global_position).normalized()
		var projectile = main_attack.instantiate() as Area2D 
		projectile.follow_target = $ProjectileMarker
		projectile.direction = direction
		projectile.global_position = $ProjectileMarker.global_position
